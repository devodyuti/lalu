...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
  	...
  	var iteration = document.querySelectorAll(".randomPizzaContainer").length
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    //if (0<sign<0.5){var move = '-'}
      //else {var move = '+'}
    //var phase = i*5;
    //items[i].style.left = items[i].basicLeft + 100 * phase + 'px';
    //items[i].style.transform = "translateX("+move+items[i].basicLeft + 50*phase + "px)";
    items[i].style.transform = "translateX("+500*phase+"px)";
    //items[i].style.transition = "transform ease-in";
    //items[j].style.transform = "translateX(-"+items[j].basicLeft + 500*phase + "px)";
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 25; i++) {
    ...
    elem.style.right=(Math.floor(i / cols) * s) + 'px';
